...
void main() {
  ...
  setUp(() {
    ...
    repository = TransactionRepositoryImpl(remote: mockIncomeDataSource);
  });
  ...
  test('Should call addTransaction on datasource', () async {
    ...
    await repository.addTransaction(TransactionType.income, income);

    verify(
      () =>
          mockIncomeDataSource.addTransactions(TransactionType.income, income),
    ).called(1);
  ...
  test('Should throw exception when addTransaction fails', () async {
    ...
    expect(
      () => repository.addTransaction(TransactionType.income, income),
